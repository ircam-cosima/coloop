    ...
    connect(port) {
            ...
            this.serialPort.on('data', function(data) {
                ...
                if ((data.indexOf("+1") > -1) || (data.indexOf("-1") > -1)) {
                    this.serialPort.emit('buttonTurned', data);
                    if (data.indexOf("+1") > -1) this.serialPort.emit('buttonIncrimented');
                    if (data.indexOf("-1") > -1) this.serialPort.emit('buttonDecrimented');
                } else if ((data.indexOf("touch") > -1) || (data.indexOf("released") > -1)) {
                    this.serialPort.emit('buttonClick', data);
                    if (data.indexOf("touch") > -1) this.serialPort.emit('buttonTouch');
                    if (data.indexOf("released") > -1) this.serialPort.emit('buttonReleased');
                } else if (data.indexOf("°C") > -1) {
                    this.serialPort.emit('temperature', parseInt(data));
                } else if (data.indexOf("Error")) {
                    this.serialPort.emit('error', data);
                }
                */
                console.log(data);
            });
    ...
    */
    ...
    allPixels(hexColor) {
        if (isHex(hexColor))
            ...
            this.serialPort.emit('error', "Error!"+hexColor+" is not valid hex number. Use this format : 0xFFFFFF");
    }
    ...
    pixel(led, hexColor) {
        if ((led >= 0) && (led <= 95)) { 
            ...
            this.serialPort.emit('error', "Error! Pixel number is out of scope! Pixels permitted : 0-95");
        }
    ...
    line (lineNumber, hexColor) {
        if ((lineNumber >= 0) && (lineNumber <= 23)) { 
            ...
            this.serialPort.emit('error', "Error! Line number is out of scope! Lines permitted : 0-23");
        }
    }
    ...
    segment (segmentNumber, hexColor) {
        if ((segmentNumber >= 0) && (segmentNumber <= 7)) { 
            ...
            this.serialPort.emit('error', "Error! Segment number is out of scope! Segments permitted : 0-7");
        }
    }
    ...
    circle (circleNumber, hexColor) {
        if ((circleNumber >= 0) && (circleNumber <= 3)) { 
            ...
            this.serialPort.emit('error', "Error! Circle number is out of scope! Circles permitted : 0-3");
        }
    }
